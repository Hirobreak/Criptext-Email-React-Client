...
const getEmailByParams = async params => {
  return await Email().findAll({ where: params });
};
...
const getEmailsToDeleteByThreadIdAndLabelId = async (threadIds, labelId) => {
  const sequelize = getDB();
  const labelIdsToDelete = labelId
    ? [labelId]
    : [systemLabels.spam.id, systemLabels.trash.id];
  return await Email().findAll({
    attributes: [[sequelize.fn('GROUP_CONCAT', sequelize.col('key')), 'key'], 'id', 'threadId'],
    include: [
      {
        model: EmailLabel(),
    ...
    return rows.map(row => ({
      ...
      key: row.key ? row.key.split(',').map(Number) : []
    }));
  });
...
const getAllLabels = async () => {
  return await Label().findAll({ raw: true });
...
const updateLabel = async ({ id, color, text, visible }) => {
  const params = {};
