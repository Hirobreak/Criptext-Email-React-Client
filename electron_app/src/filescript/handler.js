...
const { remove, encryptStreamFile, decryptEncryptStreamFile, fileStat, readDir, sendMessage } = require('./helpers');
...
const start = async () => {
  ...
  const success = await convertFilesInFolder(inputPath, outputPath, password, oldPassword);
  if (!success) {
    sendMessage('abort');
  }
