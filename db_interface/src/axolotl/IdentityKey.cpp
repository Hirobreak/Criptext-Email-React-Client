...
bool CriptextDB::createIdentityKey(database db, int accountId, string recipientId, int deviceId, char *identityKey) {
  try {
    bool hasRow = false;
    db << "begin;";
    db << "Select * from identitykeyrecord recipientId == ? and deviceId == ? and accountId == ?;"
     << recipientId
     << deviceId
     << accountId
     >> [&] (string recipientId, int deviceId, string identity) {
    ...
    if (hasRow) {
      db << "update identitykeyrecord set identityKey = ? recipientId == ? and deviceId == ? and accountId == ?;"
        << identityKey
        << recipientId
        << deviceId
        << accountId;
